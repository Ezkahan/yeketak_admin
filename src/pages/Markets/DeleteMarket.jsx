...
import { deleteMarket } from "api/services/MarketService";

const DeleteMarket = ({ market, marketDeleteHandler }) => {
  const marketDelete = () => {
    deleteMarket(market.id)
      .then((res) => {
        marketDeleteHandler(null);
        console.log(res);
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <main className="flex flex-col justify-between h-full font-montserrat-medium">
      ...
      <footer className="flex items-center justify-between">
        ...
        <button
          onClick={() => marketDeleteHandler(null)}
